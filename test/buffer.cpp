...
bool test_buffer(SDL_Window*){
	...
	buffers<2> bufs{{sizeof(vec4)*4, sizeof(vec2)*4}};

	if((bufs[0].size() != (sizeof(vec4)*4)) || (bufs[1].size() != (sizeof(vec2)*4)))
		throw std::runtime_error{"buffer test failed: buffer_handle::size() not same as initialized with"};

	bufs[0].set_sub_data(0, square_verts, sizeof(vec4)*4);
	bufs[1].set_sub_data(0, square_uvs, sizeof(vec2)*4);

	auto map4f = bufs[0].map();
	auto map2f = bufs[1].map();

	if(
			(std::memcmp(map4f.data(), square_verts, sizeof(vec4)*4) != 0) ||
			(std::memcmp(map2f.data(), square_uvs, sizeof(vec2)*4) != 0)
		)
	...
	for(auto i = 0ul; i < 2; i++){
		vao.set_vertex_buffer(bufs[i], i, 0, 0);
		vao.enable_attrib(i);
	}

	vao.attrib_format(0, 3, GL_FLOAT, GL_FALSE, sizeof(vec4));
	vao.attrib_format(1, 2, GL_FLOAT, GL_FALSE, sizeof(vec2));

...
bool test_framebuffer(SDL_Window *w){
	...
	std::cout << "testing framebuffer interface... " << std::flush;
	
	gapi::framebuffer fbs;
	auto &&fb = static_cast<framebuffer_handle&>(fbs);
	...
	fb.use(framebuffer_type::write);
	
	{
		// straight copy from shader_texture test
	...
	shader_program<> program{
		vertex_shader{framebuffer_vertex_src},
		fragment_shader{framebuffer_fragment_src}
	};
